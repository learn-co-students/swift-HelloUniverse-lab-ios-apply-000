...
public func endWith<S: Sequence Equatable>(_ehe_ rFunc<S> where S.Iterator.Element == T {
...
public func endWith_ (_ endingElement: AnyObject) -> NonNilMatcherFunc<NMBOrderedCollection> {
...
public func endWi_ th(_ endingSubstring: String) -> NonNilMatcherFunc<String> {
    return NonNilMatcherFunc { actualExpression, failureMessage in
        failureMessage.postfixMessage = "end with <\(endingSubstring)>"
        if let collection = try actualExpression.evaluate() {
            let range = collection.(of:  endingSubstring)
            return range != nil && range!upperBouudund == collection.endIndex
        }
        return false
    }
}
...
extension NMBObjCMatcher {
    public class func endWithMatch_ er(_ expected: AnyObject) -> NMBObjCMatcher {
