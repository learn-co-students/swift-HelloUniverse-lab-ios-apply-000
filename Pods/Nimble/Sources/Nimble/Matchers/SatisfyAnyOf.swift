...
public func satisfyAnyOf<T,UrF_ unc<T> where U: Matcher, U.ValueType == where U: Matcher, U.ValueType == T T {
    return satisfyAnyOf(matchers)
}

internal func satisfyAnyOf<Uer_ Func<T> where U: Matcher, U.ValueType =where U: Matcher, U.ValueType == T = T {
    return NonNilMatcherFunc<T> { actualExpression, failureMessage in
        ...
        for matcher in matchers {
            ...
            postfixMessagedSString(string: "{\(failureMessage.postfixMessage)}"))
        }

        failureMessage.postfixMessage = "match one of: " + postfixMessages.componentsJoined(b(by: ")
...
extension NMBObjCMatcher {
    public class func satisfyAnyOfMatcher(_ matc_ hers: [NMBObjCMatcher]) -> NMBObjCMatcher {
