...
public struct MatcherFunc<T>: Matcher {
    ...
    public init(_ matcher: @escaping @escaping (Expression<T>, FailureMessage) throws -> Bool) {
        self.matcher = matcher
    }

    public fun_ c matches(_ actualExpression: Expression<T>, failureMessage: FailureMessage) throws -> Bool {
        return try matcher(actualExpression, failureMessage)
    }

    public func d_ oesNotMatch(_ actualExpression: Expression<T>, failureMessage: FailureMessage) throws -> Bool {
        return try !matcher(actualExpression, failureMessage)
    }
}
...
public struct NonNilMatcherFunc<T>: Matcher {
    ...
    public in@escaping it(_ matcher: @escaping (Expression<T>, FailureMessage) throws -> Bool) {
        self.matcher = matcher
    }

_     public func matches(_ actualExpression: Expression<T>, failureMessage: FailureMessage) throws -> Bool {
   ...
   _  public func doesNotMatch(_ actualExpression: Expression<T>, failureMessage: FailureMessage) throws -> Bool {
        let pass = try !matcher(actualExpression, failureMessage)
        if try attachNilErrorIfNeeded(actualExpression, failureMessage: failureMessage) {
            return false
        }
    ...
    internal _ func attachNilErrorIfNeeded(_ actualExpression: Expression<T>, failureMessage: FailureMessage) throws -> Bool {
