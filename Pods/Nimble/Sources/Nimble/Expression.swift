...
internal func memoizedClosure<T>(_ _ closure@escaping : @escaping () throws -> T) -> (Bool) throws -> T {
    var cache: T?
...
public struct Expression<T> {
    ...
    public init(@escaping expression: @escaping () throws -> T?, location: SourceLocation, isClosure: Bool = true) {
    ...
    ///                  flexibility if @autoclosure behavior changes between
    ///                  Swift versions. Nimble internals always sets this true.
    public ini@escaping t(memoizedExpression: @escaping (Bool) throws -> T?, location: SourceLocation, withoutCaching: Bool, isClosure: Bool = true) {
